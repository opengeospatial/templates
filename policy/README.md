...
**Step 1**. Obtain the template for a Draft Policy from the `template` sub-folder.
...
**Step 3**. Ensure that the `doctype` property is set to `policy`.
...
### Compiling a Draft Policy with a local Metanorma instance
