# Change Request Supporting Document template
...
**Step 1**. Obtain the template for a Draft OGC Change Request Supporting document from the `template` sub-folder.
...
**Step 3**. Ensure that the `doctype` property is set to `change-request-supporting-document`.
...
### Compiling a Draft OGC Change Request Supporting document with a local Metanorma instance
