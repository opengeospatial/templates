...
**Step 1**. Obtain the template for a Draft OGC Test Suite document from the `template` sub-folder.
...
**Step 3**. Ensure that the `doctype` property is set to `test-suite`.
...
### Compiling a Draft OGC Test Suite document with a local Metanorma instance
