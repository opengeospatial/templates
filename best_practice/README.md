...
**Step 1**. Obtain the template for a Draft Best Practice document from the `template` sub-folder.

### Editing a Draft Best Practice document for compilation with Metanorma
...
**Step 3**. Ensure that the `doctype` property is set to `best-practice`.
...
### Compiling a Draft Best Practice document with a local Metanorma instance
...
### Compiling a Draft OGC Best Practice document with a docker-containerized Metanorma instance
