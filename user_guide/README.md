...
**Step 1**. Obtain the template for a Draft OGC User Guide document from the `template` sub-folder.
...
**Step 3**. Ensure that the `doctype` property is set to `user-guide`.
...
### Compiling a Draft OGC User Guide document with a local Metanorma instance
...
`docker run --rm -v "$(pwd)":/metanorma metanorma/metanorma  metanorma new -d user-guide -t ogc  -l https://github.com/metanorma/metanorma-templates-ogc folder_for_user_guide`
